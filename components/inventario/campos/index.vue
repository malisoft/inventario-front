<template>
	<v-card flat>
		<v-data-table :headers="headers" :items="campos" sort-by="nombre" class="elevation-1">
			<template v-slot:top>
				<v-toolbar flat color="white">
					<v-dialog v-model="dialog" max-width="600px">
						<template v-slot:activator="{ on, attrs }">
							<v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on">Nuevo Campo</v-btn>
						</template>
						<v-card>
							<v-card-title>
								<span class="headline">{{ formTitle }}</span>
							</v-card-title>

							<v-card-text>
								<v-container>
									<v-row>
										<v-col cols="12" sm="5" md="5">
											<v-text-field v-model="editedItem.nombre" label="Nombre"></v-text-field>
										</v-col>
										<v-col cols="12" sm="5" md="5">
											<v-select
												v-model="editedItem.categoria"
												:items="categorias"
												:hint="`Descripción de Categoria: ${editedItem.categoria.descripcion}`"
												item-text="nombre"
												item-value="_id"
												label="Categoria"
												persistent-hint
												return-object
												single-line
											></v-select>
										</v-col>
										<v-col cols="12" sm="2" md="2">
											<CategoriaCrear :categorias="categorias" :categoria="editedItem.categoria"/>
										</v-col>
                                        <v-col cols="12" sm="6" md="6">
											<v-select
												v-model="editedItem.tipo"
												:items="itemsTipo"
												:hint="`Campo de Tipo: ${editedItem.tipo==='TEXTO'?'Texto':'Numerico'}`"
												label="Tipo de Campo"
												persistent-hint
												return-object
												single-line
											></v-select>
										</v-col>
                                        <v-col cols="12" sm="6" md="6">
											<v-switch v-model="editedItem.estado" :label="`Estado: ${editedItem.estado?'Activado':'Desactivado'}`"></v-switch>
										</v-col>
									</v-row>
								</v-container>
							</v-card-text>
							
							<small class="mx-3 red--text">{{message}}</small>

							<v-card-actions>
								<v-spacer></v-spacer>
								<v-btn color="blue darken-1" text @click="close">Cancel</v-btn>
								<v-btn color="blue darken-1" text @click="save">Save</v-btn>
							</v-card-actions>
						</v-card>
					</v-dialog>
				</v-toolbar>
			</template>
			<template v-slot:item.estado="{ item }">
					<v-chip v-if="item.estado" color="primary">Activado</v-chip>
					<v-chip v-else color="error">Desactivado</v-chip>
			</template>
			<template v-slot:item.actions="{ item }">
				<v-chip  v-if="!item.estado" color="primary" @click="activateItem(item)">
					<v-icon>mdi-check</v-icon>Activar
				</v-chip>
				<v-chip v-else color="error" @click="deactivateItem(item)">
					<v-icon>mdi-window-close</v-icon>Desactivar
				</v-chip>
				<v-btn fab dark small color="blue" class="ml-2 mr-2" @click="editItem(item)">
                    <v-icon>mdi-pencil</v-icon>         
                </v-btn>
				<v-btn fab small color="error" @click="deleteItem(item)">
                    <v-icon>
                        mdi-delete
                    </v-icon>
                </v-btn>
			</template>
			<template v-slot:no-data>
				<v-btn color="primary" :loading="loading" @click="initialize">Actualizar</v-btn>
			</template>
		</v-data-table>
	</v-card>
</template>
<script>
import CategoriaCrear from "../categoria/crear"
export default {
	components:{
		CategoriaCrear
	},
	data: () => ({
		dialog: false,
		loading:false,
		headers: [
			{ text: 'Campo', value: 'nombre', align: 'start' },
			{ text: 'Tipo', value: 'tipo' },
			{ text: 'Categoria', value: 'categoria.nombre' },
			{
				text: 'Estado',

				sortable: false,
				value: 'estado',
			},
			{ text: 'Opciones', value: 'actions', sortable: false },
		],
        itemsTipo:['TEXTO','NUMERO'],
        campos:[],
		categorias: [],
		editedIndex: -1,
		editedItem: {
            nombre: '',
            tipo:'TEXTO',
            estado:true,
            categoria:{
                _id:'',
                nombre:'',
                descripcion:''
            }
		},
		defaultItem: {
			nombre: '',
            tipo:'TEXTO',
            estado:true,
            categoria:{
                _id:'',
                nombre:'',
                descripcion:''
            }
		},
		message:null,
	}),

	computed: {
		formTitle() {
			return this.editedIndex === -1 ? 'Nuevo Campo' : 'Editar Campo';
		},
	},

	watch: {
		dialog(val) {
			val || this.close();
		},
	},

	created() {
        this.initialize();
        this.cargarCategorias();
	},

	methods: {
		initialize() {
			this.loading=true;
			this.$axios
				.post('/campo/listar')
				.then((response) => {
					return response.data;
				})
				.then((data) => {
					this.campos = data;
					this.loading=false;
				})
				.catch((error) => {
					this.loading=false;
				});
        },
        cargarCategorias() {
			this.message = null;
			this.$axios
				.post('/categoria/listar', { estado: true })
				.then((response) => {
					return response.data;
				})
				.then((data) => {
					this.categorias = data;
				})
				.catch((error) => {
					this.message = 'Ocurrió un error al cargar Categorias.';
				});
		},

        activateItem(item){
            this.$axios
					.put('/campo/activar', { _id: item._id })
					.then((res) => {
						return res.data;
					})
					.then((data) => {
						item.estado=true;
					})
					.catch((error) => {
						confirm('Ocurrió un problema al activar.');
					});
        },
        deactivateItem(item){
            this.$axios
					.put('/campo/desactivar', { _id: item._id })
					.then((res) => {
						return res.data;
					})
					.then((data) => {
						item.estado=false;
					})
					.catch((error) => {
						confirm('Ocurrió un problema al desactivar.');
					});
        },

		editItem(item) {
			this.editedIndex = this.campos.indexOf(item);
			this.editedItem = Object.assign({}, item);
			this.dialog = true;
		},

		deleteItem(item) {
			const index = this.campos.indexOf(item);
			if (confirm('Seguro que deseas eliminar el Campo?')) {
				this.$axios
					.delete('/campo/eliminar', { _id: this.campos[index]._id })
					.then((res) => {
						return res.data;
					})
					.then((data) => {
						this.campos.splice(index, 1);
					})
					.catch((error) => {
						confirm('Ocurrió un problema al Eliminar.');
					});
			}
		},

		close() {
			this.dialog = false;
			this.$nextTick(() => {
				this.editedItem = Object.assign({}, this.defaultItem);
				this.editedIndex = -1;
			});
		},

		save() {
			if (this.editedIndex > -1) {
				this.$axios
					.post('/campo/actualizar', this.editedItem)
					.then((res) => {
						return res.data;
					})
					.then((data) => {
						Object.assign(this.campos[this.editedIndex], data);
						this.close();
					})
					.catch((error) => {
						this.messageDialog = 'Ocurrió un prolema al crear un nueva Categoria.';
					});
			} else {
				this.$axios
					.post('/campo/crear', this.editedItem)
					.then((res) => {
						return res.data;
					})
					.then((data) => {
						this.campos.push(data);
						this.close();
					})
					.catch((error) => {
						this.messageDialog = 'Ocurrió un prolema al crear un nueva Categoria.';
					});
			}
		},
	},
};
</script>