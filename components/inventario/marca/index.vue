<template>
	<v-card flat>
		<v-data-table :headers="headers" :items="marcas" sort-by="nombre" class="elevation-1">
			<template v-slot:top>
				<v-toolbar flat color="white">
					<v-dialog v-model="dialog" max-width="600px" persistent>
						<template v-slot:activator="{ on, attrs }">
							<v-btn color="primary" dark class="mb-2" v-bind="attrs" v-on="on">Nueva Marca</v-btn>
						</template>
						<v-card>
							<v-card-title>
							...
							<v-card-text>
								...
								<Modelo :_id="editedItem._id" :modelos="modelos" :editedMarca="editedIndex" />
							</v-card-text>
...
export default {
	...
	methods: {
		...
		save() {
			this.message = null;
			this.editedItem.modelos=this.modelos;
			if (this.editedIndex > -1) {
				this.$axios
					.put('/marca/actualizar', this.editedItem)
					.then((response) => {
						return response.data;
					})
					.then((data) => {
						Object.assign(this.marcas[this.editedIndex], data);
						this.close();
					})
					.catch((error) => {
						this.message = 'Ocurrio un error';
